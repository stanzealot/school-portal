...
import { SearchBar, FilterDropdown, SemesterBadge, Pagination, EmptyState, TableCard, IconBtn } from '@/components/ui/exam-officer';
import { MOCK_STUDENTS, EXAM_DEPARTMENTS, EXAM_PAGE_SIZE } from '@/constants/exam-officer-mock';
import { MOCK_CURRICULUM } from '@/constants/exam-officer-mock';
import { COLORS } from '@/constants/theme.constants';
import { Eye } from 'lucide-react';

// Simulate carryover: students with ND2/HND1/HND2 level
const CARRYOVER_DATA = MOCK_STUDENTS.filter(s => s.level !== 'ND1').map(s => {
  const course = MOCK_CURRICULUM[Math.floor(Math.random() * MOCK_CURRICULUM.length)];
  return { ...s, carryoverCourse: course.courseCode, courseTitle: course.courseTitle, semester: course.semester };
});

const StudentCarryoverPage = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return CARRYOVER_DATA.filter(s =>
  ...
  return (
    <TableCard>
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 gap-3 flex-wrap">
        <h3 className="font-bold text-base" style={{ color: COLORS.text.title }}>Student Carryover</h3>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50/60">
              ...
              <th className="px-5 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">Course Title</th>
              <th className="px-5 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">Semester</th>
          ...
          <tbody>
            {paginated.length === 0 ? (
              <tr><td colSpan={8}><EmptyState message="No carryover records found" /></td></tr>
            ) : paginated.map((s, i) => (
              <motion.tr key={`${s.id}-${i}`} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.15, delay: i * 0.03 }}
                ...
                <td className="px-5 py-3.5 text-gray-600">{s.carryoverCourse}</td>
                <td className="px-5 py-3.5 text-gray-600">{s.courseTitle}</td>
                <td className="px-5 py-3.5"><SemesterBadge semester={s.semester} /></td>
                <td className="px-5 py-3.5 text-gray-500">{s.level}</td>
                <td className="px-5 py-3.5"><IconBtn onClick={() => {}} title="View"><Eye size={15} /></IconBtn></td>
              </motion.tr>
            ))}
          </tbody>
        </table>
...
export default StudentCarryoverPage;
