...
import { Filter, ChevronDown, ChevronLeft, ChevronRight, Eye, Upload, Search, X } from 'lucide-react';
import { cn } from '@/utils';
import { COLORS } from '@/constants/theme.constants';
import { useCourses } from '@/hooks/api/useCourses';
import { mapApiCourseToAttendance } from '@/lib/api/mapCourse';
import { getApiErrorMessage } from '@/lib/api/errors';
import { semesterLabelToApi, sessionLabelToSessionId } from '@/lib/session';
import {
...
const ResultList = ({ onView }: { onView: (c: AttendanceCourse) => void }) => {
  ...
  const [filterSession, setFilterSession] = useState(SESSIONS[0] ?? '');
  ...
  const courseParams = useMemo(() => {
    if (!filterSession || !filterSemester) return null;
    return {
      sessionId: sessionLabelToSessionId(filterSession),
      semester: semesterLabelToApi(filterSemester),
      page,
      limit: PAGE_SIZE,
    };
  }, [filterSession, filterSemester, page]);

  const { data: coursesPayload, isLoading, isError, error, refetch, isFetching } = useCourses(courseParams);

  const apiRows = useMemo(() => {
    const raw = coursesPayload?.data;
    if (!Array.isArray(raw)) return [];
    return raw.map((item) => mapApiCourseToAttendance(item as Record<string, unknown>));
  }, [coursesPayload]);

  const filtered = useMemo(() => {
    if (!filterDept) return apiRows;
    return apiRows.filter((c) => c.department === filterDept);
  }, [apiRows, filterDept]);

  const totalPages = Math.max(1, coursesPayload?.pagination?.totalPages ?? 1);
  ...
  return (
    <div className="flex flex-col gap-5 max-w-[1100px] mx-auto w-full">
      {isError && (
        <div
          className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 rounded-[4px] border border-red-100 bg-red-50 px-4 py-3 text-sm text-red-800"
          role="alert"
        >
          ...
          <button
            type="button"
            onClick={() => refetch()}
            className="shrink-0 rounded-[4px] px-3 py-1.5 font-semibold text-white self-start sm:self-auto"
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <h3 className="font-bold text-base" style={{ color: COLORS.text.title }}>Students Result</h3>
          {/* Desktop filters */}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              ) : isError ? null : paginated.length === 0 ? (
                <tr><td colSpan={9}><EmptyState /></td></tr>
              ) : (
                paginated.map((course, i) => (
                  <ResultRow key={`${course.courseCode}-${i}`} course={course} index={i} onView={onView} />
                ))
              )}
            </tbody>
          </table>
